...
import { crmApi, type ClienteCrm } from './crm';
...
function normaliza(raw: any): ClienteCrm {
  ...
  return {
    ...
    data_ultimo_pedido: raw.data_ultimo_pedido
      ? String(raw.data_ultimo_pedido).slice(0, 10)
      : null,
...
export function useCrmClientes() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await crmApi.listaClientes();
      const lista = (res.clientes || []).map(normaliza);
      setClientes(lista);
  ...
  return { clientes, total, loading, error, refresh };
}
